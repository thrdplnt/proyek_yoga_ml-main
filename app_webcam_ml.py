...
# 1. URUTAN POSE UNTUK SETIAP SLOT
POSE_SEQUENCE = [
    "plank", "tree_pose", "downdog", "cobra", "half_moon", "breath_of_fire"
]
MAX_PHOTOS = len(POSE_SEQUENCE)

# 2. AMBANG BATAS KEYAKINAN MODEL
CONFIDENCE_THRESHOLD = 0.64 # Sesuai permintaan terakhir (64%)

# 3. PENGHITUNG STABILITAS
# Jumlah frame di mana pose harus benar sebelum countdown dimulai
STABILITY_FRAMES = 15 # Sekitar 1 detik, bisa dinaikkan jika ingin lebih lama
...
try:
    ...
    st.warning("Pastikan file model (.pkl) berada di folder yang sama dengan script ini.")
    st.stop()
...
if 'stability_counter' not in st.session_state: st.session_state.stability_counter = 0
...
if not st.session_state.camera_active:
    if not st.session_state.camera_indices:
        ...
        st.session_state.selected_camera = st.selectbox("Pilih Kamera Anda:", options=st.session_state.camera_indices, format_func=lambda x: f"Kamera {x}")
...
with col2:
    if st.button("🔄 Ulangi Sesi", use_container_width=True):
        st.session_state.camera_active = False; st.session_state.active_slot = 0; st.session_state.captured_photos = [None] * MAX_PHOTOS; st.session_state.photobooth_complete = False; st.session_state.countdown = -1; st.session_state.stability_counter = 0; release_camera(); st.rerun()

if not st.session_state.photobooth_complete:
    target_pose_name = POSE_SEQUENCE[st.session_state.active_slot]
    st.header(f"SLOT {st.session_state.active_slot + 1}/{MAX_PHOTOS}: Tahan Pose '{target_pose_name.upper()}'")
else:
...
if st.session_state.camera_active and not st.session_state.photobooth_complete:
        ...
        while cap.isOpened() and st.session_state.camera_active:
            ...
            if results.pose_landmarks:
                ...
                try:
                    ...
                    if detected_pose.lower() == target_pose_name.lower() and proba > CONFIDENCE_THRESHOLD:
                        is_pose_correct = True
                except Exception: pass

            # Logika Penghitung Stabilitas
            if is_pose_correct:
                st.session_state.stability_counter += 1
            else:
                st.session_state.stability_counter = 0
            
            # Memulai Countdown jika pose sudah stabil
            if st.session_state.stability_counter >= STABILITY_FRAMES and st.session_state.countdown == -1:
                st.session_state.countdown = 3
                st.session_state.countdown_time = time.time()
                st.session_state.stability_counter = 0

            # Tampilan Debugging di Video
            cv2.rectangle(display_frame, (0, 0), (550, 130), (20, 20, 20), -1)
            cv2.putText(display_frame, f"TARGET: {target_pose_name.upper()}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
            color_detected = (0, 255, 0) if is_pose_correct else (255, 255, 255)
            cv2.putText(display_frame, f"TERDETEKSI: {detected_pose.upper()}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color_detected, 2)
            cv2.putText(display_frame, f"KEYAKINAN: {int(proba*100)}%", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color_detected, 2)
            stability_progress = min(st.session_state.stability_counter, STABILITY_FRAMES)
            cv2.putText(display_frame, f"STABIL: [{stability_progress}/{STABILITY_FRAMES}]", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color_detected, 2)

            # Logika Countdown
            if st.session_state.countdown > -1:
                if st.session_state.countdown > 0:
                    text = str(st.session_state.countdown)
                    cv2.putText(display_frame, text, (int(display_frame.shape[1]/2)-50, int(display_frame.shape[0]/2)+50), cv2.FONT_HERSHEY_SIMPLEX, 10, (0, 255, 0), 25)
                if time.time() - st.session_state.countdown_time >= 1:
                    st.session_state.countdown -= 1
                    st.session_state.countdown_time = time.time()
                if st.session_state.countdown == 0:
                    st.toast(f"Pose '{target_pose_name}' Berhasil!")
                    active_slot = st.session_state.active_slot
                    clean_frame_for_photo = cv2.flip(raw_frame, 1)
                    pil_cap_rgb = Image.fromarray(cv2.cvtColor(clean_frame_for_photo, cv2.COLOR_BGR2RGB))
                    
                    x_slot, y_slot, w_slot, h_slot = slot_definitions[active_slot]
                    st.session_state.captured_photos[active_slot] = crop_and_resize_image(pil_cap_rgb, w_slot, h_slot)
                    
                    st.session_state.active_slot += 1
                    st.session_state.countdown = -1

                    if st.session_state.active_slot >= MAX_PHOTOS:
                        st.session_state.photobooth_complete = True
    ...
    if final_image:
        live_preview_placeholder.image(final_image, use_container_width=True)

if st.session_state.photobooth_complete:
    st.balloons()
    buf = io.BytesIO()
    final_image.save(buf, format="PNG")
    st.download_button("📥 Unduh Hasil Photostrip", buf.getvalue(), "photostrip_yoga.png", "image/png", use_container_width=True)